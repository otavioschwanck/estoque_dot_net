...
USE `banco_estoque`$$
CREATE TRIGGER `banco_estoque`.`entrada_AFTER_DELETE` AFTER DELETE ON `entrada` FOR EACH ROW
BEGIN
	declare valor decimal;
	set valor = (select valor_venda from produtos where id_produtos = old.produtos_id_produtos limit 1);
	update caixa set em_caixa = em_caixa + (old.qtd * valor) where id_caixa = old.caixa_id_caixa;
	update produtos set qtd = qtd - old.qtd where id_produtos = old.produtos_id_produtos;
END
$$
...
USE `banco_estoque`$$
CREATE TRIGGER `banco_estoque`.`saida_AFTER_DELETE` AFTER DELETE ON `saida` FOR EACH ROW
BEGIN
	declare valor decimal;
		set valor = (select valor_compra from produtos where id_produtos = old.produtos_id_produtos limit 1);
		update caixa set em_caixa = em_caixa - (old.qtd * valor) where id_caixa = old.caixa_id_caixa;
		update produtos set qtd = qtd + old.qtd where id_produtos = old.produtos_id_produtos;
END
$$
